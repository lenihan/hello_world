...
function build_third_party_dependencies {
    ...
    $packages = 
        ...
        "imgui",                            # Windows: ~22 s, WSL:    ~2 m, Ubuntu:  ~9 s
        "magnum",                           # Windows:  ~3 m, WSL:   ~22 m, Ubuntu:  ~2 m
        "osg[tools,plugins,examples]",      # Windows: ~30 m, WSL:  ~1.1 h, Ubuntu: ~15 m
        "qt5"                               # Windows: ~60 m, WSL: ~10.2 h, Ubuntu: ~54 m
    foreach ($pkg in $packages) {
        ./vcpkg --triplet=$triplet --recurse install $pkg
    }
